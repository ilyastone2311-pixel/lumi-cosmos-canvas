...
const SearchBar = ({ isOpen, onClose }: SearchBarProps) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-start justify-center pt-24 px-4">
      ...
      <div 
        className="relative w-full max-w-2xl rounded-2xl overflow-hidden animate-scale-in bg-card border border-border"
        style={{
          boxShadow: '0 20px 60px hsl(var(--foreground) / 0.15), 0 0 0 1px hsl(var(--primary) / 0.1)',
        }}
      >
        {/* Search Input */}
              ...
              <div className="flex flex-wrap gap-2">
                {trendingSearches.map((term) => (
                  <button
                    ...
                    className="px-4 py-2 rounded-full bg-muted hover:bg-muted/80 text-sm text-muted-foreground hover:text-foreground transition-colors border border-border/50"
                  >
                    {term}
                  </button>
                ))}
