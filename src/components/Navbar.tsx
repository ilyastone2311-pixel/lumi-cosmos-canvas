...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 w-full py-2 px-4">
      <div
      ...
      >
        <div className="relative px-3 sm:px-4 py-0.5 flex items-center justify-between">
          ...
          <motion.div
            ...
            whileTap={{ scale: 0.98 }}
          >
            <motion.div className="relative flex-shrink-0 overflow-hidden origin-center -my-1" transition={{ duration: 0.2 }}>
              <img
                src={logoFullImage}
                alt="Lumi logo"
                className="h-14 sm:h-16 md:h-[3.25rem] w-auto object-contain relative z-10 block"
              />
              <div
            ...
            {user && (
              <motion.button
                ...
                className={`relative px-5 py-2 rounded-full font-display font-medium text-base transition-all duration-300 group ${
                  isActive("/recommended") ? "text-foreground" : "text-muted-foreground hover:text-foreground"
                }`}
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              >
